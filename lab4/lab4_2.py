...
while True :
	...
	if current_time < 20:
		...
		if dist1 > dist2:
			if abs(u) > 30:
				mB.run_direct(duty_cycle_sp = 70+math.copysign(30,u))
				mC.run_direct(duty_cycle_sp = 70-math.copysign(30,u))
				fh.write(str(current_time) + ' ' + str(d) + '\n')
			else:
				mB.run_direct(duty_cycle_sp = 70+u)
				mC.run_direct(duty_cycle_sp = 70-u)
				fh.write(str(current_time) + ' ' + str(d) + '\n')
		else:
			if abs(u) > 30:
				mB.run_direct(duty_cycle_sp = 70-math.copysign(30,u))
